...
class TestIssuesCrudAndAuthorization:
    ...
    @pytest.mark.django_db
    def test_authenticated_user_create_issue_when_author_or_contributor_of_project_with_assigned_user(
        self,
    ):
        """
        ...
        url = reverse("issues", kwargs={"pk": 1})
        response = client.post(
            url, data=self.issue_data2, content_type="application/json", headers=headers
        )
        assert response.status_code == 200

    @pytest.mark.django_db
    def test_authenticated_user_create_issue_when_author_or_contributor_of_project_without_assigned_user(
        self,
    ):
        """
        Ensure an authenticated user can create an issue in a project which he is the author or contributor
        without have to specify an assigned user.
        """
        client = Client()
        url = reverse("signup")
        client.post(url, data=self.user_data1)
        ...
        url = reverse("issues", kwargs={"pk": 1})
        response = client.post(
            url, data=self.issue_data0, content_type="application/json", headers=headers
        )
        assert response.status_code == 200
    ...
    def test_authenticated_user_create_issue_but_with_wrong_data(
        self, balise, priority, status
    ):
        """
    ...
    @pytest.mark.django_db
    def test_authenticated_user_create_issue_when_not_author_or_contributor_of_project(
        self,
    ):
        """
    ...
    def test_remove_user_from_project_where_he_is_assigned_to_issue(self):
        ...
        response = client.post(
            url,
            data=self.contributor_project2a,
            content_type="application/json",
            headers=headers,
        )
        assert response.status_code == 200
        ...
        url = reverse("issues", kwargs={"pk": 1})
        response = client.post(
            url, data=self.issue_data3, content_type="application/json", headers=headers
        )
        assert response.status_code == 200
        ...
        url = reverse("issues_detail", kwargs={"pk": 1, "issue_id": 1})
        response = client.get(url, content_type="application/json", headers=headers)
        assert response.data["assignee_user_id"] is None
        assert response.status_code == 200

    @pytest.mark.django_db
    def test_user_delete_himself_so_assignee_id_must_be_none_where_user_contribute_as_an_assignee(
        self,
    ):
        """
        ...
        url = reverse("login")
        data = {
            "username": self.user_data4["username"],
            "password": self.user_data4["password"],
        }
        response = client.post(url, data=data)
    ...
    @pytest.mark.django_db
    def test_once_assignee_id_is_none_only_a_contributor_can_be_assginee_lets_try_missing_contributor(
        self,
    ):
        """
        When an authenticated user which delete his account we ensure the contributions which he is an assignee
    ...
    @pytest.mark.django_db
    def test_once_assignee_id_is_none_only_a_contributor_can_be_assginee_lets_try_valid_contributor(
        self,
    ):
        """
        When an authenticated user which delete his account we ensure the contributions which he is an assignee
        ...
        response = client.post(
            url,
            data=self.contributor_project2c,
            content_type="application/json",
            headers=headers,
        )
        assert response.status_code == 200
    ...
    @pytest.mark.django_db
    def test_authenticated_user_update_issue_status_for_which_he_is_not_assigned_but_is_the_author(
        self,
    ):
        """
    ...
    @pytest.mark.django_db
    def test_authenticated_user_update_issue_status_for_which_he_is_not_assigned_and_is_not_author(
        self,
    ):
        """
    ...
    @pytest.mark.django_db
    @pytest.mark.parametrize(
        "status", [("progressing"), ("Open"), ("Unfinished"), ("Annulé")]
    )
